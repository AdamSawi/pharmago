...
export function MessagesTab() {
  ...
  const [clientOrders, setClientOrders] = useState<Record<string, string>>({});
  ...
  async function openPicker() {
    ...
    try {
      const [pharmacies, clients] = await Promise.all([
        getUsersByRole('pharmacy'),
        getDeliveryClientContacts(user.uid),
      ]);
      setClientOrders(Object.fromEntries(clients.map((c: ChatContact) => [c.uid, c.orderId ?? ''])));
      setSegments([
        { id: 'pharmacy', label: 'Pharmacies', contacts: pharmacies.map((p) => ({ uid: p.uid, name: p.name, role: 'pharmacy' as const })) },
  ...
  async function handleSelectContact(contact: Contact) {
    ...
    const conversationId = await getOrCreateConversation({
      ...
      nameB: contact.name,
      roleB: contact.role,
      orderId: contact.role === 'client' ? (clientOrders[contact.uid] ?? null) : null,
    });
      ...
      >
        ...
        {conversations.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>Contactez une pharmacie ou un client pour démarrer une discussion.</Text>
          </View>
        ) : (
          <View style={styles.list}>
            {conversations.map((c) => (
              <ConversationRow
                ...
                conversation={c}
                currentUid={user.uid}
                onPress={() => router.push({ pathname: '/(delivery)/conversation' as never, params: { conversationId: c.id } })}
              />
            ))}
          </View>
        )}
